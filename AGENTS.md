...
- **阶段**：二次开发初期；原 upstream 为 Pipecat + Twilio 电话 bot，LLM 已切换为 DashScope Qwen Omni
- **本地入口**：`uv run bot.py`（SmallWebRTC 本地调试，http://localhost:7860）
- **环境配置**：参考 `env.example`，真实密钥写入 `.env`（不提交）
- **部署**：Pipecat Cloud，见 `pcc-deploy.toml` 与 `Dockerfile`
- **当前改动面**：LLM 对接层（`qwen_omni_live_service.py`），业务逻辑尚未开始
